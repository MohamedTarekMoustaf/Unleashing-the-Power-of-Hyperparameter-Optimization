...
Original file is located at
    https://colab.research.google.com/drive/12uP7EIDnEt86loi-zl8VfLfYdgUlyTIr
...
import numpy as np
np.random.seed(3)
lr = np.power(10, np.random.uniform(-2,0,500))
m = np.subtract(1,np.power(10, np.random.uniform(-3,-1,500)))
mbs = mini_batch_size = np.power(2, np.random.randint(4,6,500))
accuracy_list=list()
for i in range(500):

 parameters = model(train_X, train_Y, layers_dims, beta = m[i],learning_rate = lr[i], mini_batch_size = mbs[i], optimizer = "momentum")

  # Predict
 predictions = predict(train_X, train_Y, parameters)
 accuracy=np.mean((predictions[0,:] == train_Y[0,:]))
 accuracy_list.append(accuracy)

print("mini_batch_size: ",list(mbs),'\n',"learning_rate: ", list(lr),'\n',"momentum: ", list(m),'\n')
print(accuracy_list,'\n')
...
hyperparam_arr=np.stack((mbs,lr,m),axis=1)
filepath = "/content/my_drive/MyDrive/excel/models_eval acc.xlsx"
#eval_accuracy_arr=np.array(sorted(eval_accuracy_list,reverse=True))*100
eval_accuracy_arr=np.array(accuracy_list)*100
excel_mat=np.concatenate((hyperparam_arr,eval_accuracy_arr.reshape((500,1))),1) # horizontal concatenation
excel_mat=excel_mat[excel_mat[:,3].argsort()[::-1]]
excel_mat=np.float64(excel_mat)

#print(excel_mat)
df = pd.DataFrame.from_records(excel_mat)
df.to_excel(filepath, header = ['Batch_size','Learning_rate','Momentum','cross_validation accuracy (%)'], index_label = 'Models')
