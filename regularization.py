...
Original file is located at
    https://colab.research.google.com/drive/1pgkJ5iSkwMCAKyY0NLjFLuqOnxmwrAzO
...
def compute_cost_with_regularization(A3, Y, parameters, lambd):
    ...
    L2_regularization_cost=0   # initialization of L2_regularization_cost

    #(≈ 1 lines of code)
    # L2_regularization_cost =
    # YOUR CODE STARTS HERE
    for l in range(1,L+1):
        L2_regularization_cost+=np.multiply(lambd/(2*m),np.sum(np.square(parameters["W"+str(l)])))
...
import numpy as np
np.random.seed(3)
lr = np.power(10, -2 * np.random.rand(500))
k= np.power(10, -1 * np.random.rand(500))
val_accuracy_list=list()
train_accuracy_list=list()
for i in range(500):
 parameters = model(train_X, train_Y, learning_rate=lr[i], lambd = k[i])
 print ("On the train set:")
 predictions_train = predict(train_X, train_Y, parameters)
 accuracy=np.mean((predictions_train[0,:] == train_Y[0,:]))
 train_accuracy_list.append(accuracy)
 print ("On the test set:")
 predictions_test = predict(test_X, test_Y, parameters)
 accuracy=np.mean((predictions_test[0,:] == test_Y[0,:]))
...
hyperparam_arr=np.stack((lr,k),axis=1)
#print(hyperparam_arr)
filepath = "/content/my_drive/MyDrive/models_eval acc.xlsx"
#eval_accuracy_arr=np.array(sorted(eval_accuracy_list,reverse=True))*100
train_accuracy_arr=np.array(train_accuracy_list)*100
eval_accuracy_arr=np.array(val_accuracy_list)*100
excel_mat=np.concatenate((hyperparam_arr,train_accuracy_arr.reshape((500,1)),eval_accuracy_arr.reshape((500,1))),1) # horizontal concatenation
excel_mat=excel_mat[excel_mat[:,3].argsort()[::-1]]
excel_mat=np.float64(excel_mat)

print(val_accuracy_list)
df = pd.DataFrame.from_records(excel_mat)
df.to_excel(filepath, header = ['Learning_rate','Regularization_parameter','train_accuracy (%)','cross_validation accuracy (%)'], index_label = 'Models')
...
def forward_propagation_with_dropout(X, parameters, keep_prob = 0.5):
    ...
    D1 = (D1 < keep_prob).astype(int)                # convert boolean into integer
    ...
    D2 = (D2 < keep_prob).astype(int)                # convert boolean into integer
